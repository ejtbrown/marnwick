...
- Remove masked objects with local GPU-accelerated LaMa and automatic CPU fallback, or an explicitly trusted remote LaMa service.
...
Application preferences include window geometry, remembered catalogs, thumbnail columns, sort order, normal versus wipe deletion, the LaMa processing runtime, and the Remote LaMa endpoint and trusted certificate. Per-catalog preferences include saved thumbnail size and thumbnail-prune parallelism.
...
- Remote LaMa intentionally sends the prepared 512×512 crop and mask to the configured endpoint. Certificate pinning authenticates the endpoint after the user verifies its thumbprint, but it does not make an untrusted remote service private; select a remote engine only when its operator and data-handling policy are acceptable.
...
| Remote LaMa | [`src/marnwick/remote_lama.py`](src/marnwick/remote_lama.py) | Endpoint validation, certificate retrieval and exact pinning, bounded HTTPS health checks, multipart inference, and cancellation |
