...
- LaMa inference admits one request per viewer and runs in one application-owned, isolated process with a bounded timeout. The worker is warmed opportunistically and retains its initialized session across edits; cancellation, a native worker failure, or a runtime/model change recycles it. Its input and generated patch are fixed at 512 pixels per side; the full-resolution source stays in Marnwick's background preparation lane.
