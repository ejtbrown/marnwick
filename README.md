...
- Remove masked objects with local GPU-accelerated LaMa and automatic CPU fallback, or an explicitly trusted Remote GPU service.
- Find faces with local or explicitly trusted Remote GPU inference, group deep embeddings conservatively, and turn high-payoff similarity suggestions into reversible batch identity decisions.
...
shortcut that opens the dialog is configurable. Press and hold `Tab` anywhere
in Marnwick to dim the active window and show the other shortcuts available in
that view; release it to return. `Tab` is reserved for this guide and cannot be
...
with that prefix. `Ctrl+Space` completes the matching tag used most recently
during the current session, and `Enter` applies the dialog from either the
entry or a tag checkbox. Completion and **Repeat (Ctrl-R)** can be reassigned
under **Tools > Hotkeys**; Repeat replaces the pending selection with the exact
tag set from the most recent image tagged in the session, including an empty
set. Catalog-tag additions and image-tag assignments run as highest-priority
protected catalog tasks. If
...
In LaMa mode, paint the complete area to remove, use the mouse wheel to resize the mask brush, press `Backspace` to clear the mask, and press `Enter` to apply it. Overlapping brush samples are rendered as one uniformly translucent mask instead of becoming more opaque during a drag. Press `R` to repeat the last successfully applied mask; its positions and brush sizes are stored as image-relative percentages so the pattern scales across resolutions and aspect ratios. A centered progress indicator remains visible while inference runs and reports the execution provider actually in use; `Escape` cancels the mask or a running inference. Under **Tools > Preferences > LaMa**, choose **Auto**, **CPU**, **NVIDIA**, **WebGPU**, or **Remote GPU**. Auto prefers an available vendor GPU provider, then a physical WebGPU device, and finally CPU; explicit local GPU selections also retry on CPU if session creation or inference fails. WebGPU is registered as an ONNX Runtime plugin in the isolated worker and uses the platform's native GPU stack across Linux, macOS, and Windows. Local engines never upload image data. Selecting a local engine starts an opportunistic background warm-up, and one isolated worker retains the initialized model session for subsequent edits until the application closes or the runtime/model changes. Marnwick crops bounded context around the mask, retains the generated pixel patch and execution-provider record in the edit history, and does not rerun the model when saving. LaMa currently supports static images only. Choosing a local engine when its model is absent offers to download it, and **Tools > Download LaMa Model** provides the same operation on demand. Downloads are pinned and SHA-256 verified before publication.

**Tools > Remote GPU** configures the shared remote HTTPS inference service by IP address and port. **Retrieve Certificate** connects without sending image data, displays the offered certificate's SHA-1 thumbprint, and asks for explicit trust. Marnwick stores the complete trusted certificate in its private global configuration and compares the exact DER certificate before every new TLS connection sends a request; a changed certificate is rejected with both thumbprints and no image data is sent. For LaMa, Marnwick sends only the prepared 512×512 context crop and binary mask—not the full-resolution source. For face processing, it sends an EXIF-oriented image reduced to at most 1280 pixels on its longest edge, then strictly validates the returned input hash, model versions, CUDA provider, normalized detections and embeddings, and aligned crops before publishing anything to the catalog.

**Tools > GPU Test** runs the same generated 512×512 image and erase mask through every supported local GPU backend, the configured Remote GPU endpoint's LaMa service, and a CPU baseline. Isolated local processes verify the repaired output and provider assignment, while the remote test verifies the pinned certificate, endpoint health, returned PNG, changed mask, and exact preservation of unmasked decoded pixels. The comparison reports separate initialization or TLS-health setup, first-inpaint, and warmed-inpaint times; remote rows include server-reported inference time when available. Local repair timings are collected without ONNX profiling overhead, and a separate tiny diagnostic verifies local provider assignment. If the local model data is not installed yet, the existing confirmed download flow is offered first.

### People and face suggestions

Face processing is opt-in per catalog under the catalog **Preferences**. When enabled, idle indexing runs YuNet detection and SFace deep embedding after ordinary catalog indexing. Under **Tools > Preferences > Face processing**, choose **Auto**, **CPU**, **NVIDIA**, **WebGPU**, or **Remote GPU**. Local accelerated providers fall back to CPU through the same ONNX Runtime provider selection used by local LaMa. The two pinned local models total about 37 MiB, are downloaded only after confirmation, and are SHA-256 verified before atomic publication. **Tools > Download Face Models** can install or replace them explicitly; they are not required when Remote GPU is selected.

The **People** entry in the catalog tree and **Tools > People** open a keyboard-first review dialog. Its default queue ranks conservative known-person proposals and unnamed similarity groups by review payoff, shows diverse representative crops first, and lets **Review all** expand a group only when needed. **Re-evaluate unnamed faces** performs a full-corpus comparison against diverse prototypes from every confirmed identity; it stores only review proposals and never assigns an identity without confirmation. Selecting a new group focuses and selects the Name field so typing can begin immediately. With no crop selection, a decision applies to the complete group; selecting crops limits it to those faces. `Enter` confirms or names, `R` removes selected faces from the current group, `X` permanently rejects a proposed person for those faces or separates a selected subset from an unnamed group, `?` defers the decision for seven days, `I` ignores a real face, `Delete` marks a false detection, and `Space` opens the source photo fullscreen. Named people, loose faces, ignored faces, and rejected detections remain separately reviewable, and **Undo last** reverses the latest face-management decision.

Marnwick deliberately proposes similarity rather than assigning identity autonomously. Names, normalized 128-value embeddings, decision history, rejection relationships, and content-addressed face crops stay in the catalog's `.marnwick` state. Local runtimes send nothing to a service. Remote GPU receives only the bounded image needed for stateless detection and embedding; it never receives names, identities, catalog history, or known-person galleries. Re-detection preserves a user's decision when a face still overlaps the same location. **Tools > Remove Face Data** disables processing and removes all face-derived catalog state without modifying source photographs; disabling the catalog preference by itself merely pauses new analysis so it does not silently discard work.
...
└── .marnwick/
    ...
    ├── marnwick.log
    ├── timings.json
    ├── face-thumbnails/<hash-prefix>/<sha256>.jpg
    └── thumbnails/<size>/<hash-prefix>/<sha256>.jpg
```

SQLite stores image records, dimensions, hashes, similarity features, tags, catalog settings, directory freshness state, indexing failures, trash restore paths, and opt-in face embeddings and review decisions. General and face-crop JPEG thumbnails are content-addressed files outside the database. Legacy database thumbnail blobs are migrated lazily.
...
| `MARNWICK_FACE_MODEL_DIR` | Override the directory containing the downloaded YuNet and SFace ONNX models | Platform data directory |
| `MARNWICK_LAMA_RUNTIME` | Select runtimes installed by setup (`auto`, `cpu`, `gpu`, or `webgpu`; also `nvidia`/`vulkan` on Linux, `metal` on macOS, and `directml`/`d3d12` on Windows) | `auto` |
| `MARNWICK_LAMA_THREADS` | Limit LaMa worker CPU threads from 1 to 64 | Up to 8, leaving one logical CPU free |
| `MARNWICK_LAMA_TIMEOUT_SECONDS` | Bound one local or remote LaMa inference | `900` |
...
| `PYTHON` | Override the interpreter used by setup | `python3` on Linux/macOS; discovered Python on Windows |
| `MARNWICK_DEBUG_TOKEN` | Authenticate the optional localhost debug protocol | Random token printed to stderr |

Application preferences include window geometry, remembered catalogs, thumbnail columns, sort order, normal versus wipe deletion, the LaMa and face-processing runtimes, and the Remote GPU endpoint and trusted certificate. Per-catalog preferences include saved thumbnail size, thumbnail-prune parallelism, and whether face processing is enabled.
...
Face models default to the `faces` subdirectory below the same platform data root. They are pinned to a specific [OpenCV Zoo](https://github.com/opencv/opencv_zoo) revision: the YuNet detector directory is MIT-licensed and the SFace recognizer directory is Apache-2.0. Those model files are also not included in the Marnwick repository or normal package.
...
- LaMa inference admits one request per viewer and has a bounded timeout. Local inference runs in one application-owned isolated process; the worker is warmed opportunistically and retains its initialized session across edits, while cancellation, a native worker failure, or a runtime/model change recycles it. Remote inference uses a cancelable pinned-TLS request on the viewer's bounded background lane. Both paths use a 512×512 input and generated patch; the full-resolution source stays in Marnwick's background preparation lane.
- Face indexing is a lower-priority idle action than catalog and thumbnail indexing. It warms one detector/embedding pair per pass, bounds detector input to 1280 pixels on its longest edge and each review queue to 500 groups, and keeps database reads and face-crop loading off the Qt thread. Large known-person searches use deterministic locality-sensitive buckets and a bounded number of high-quality anchors per person before exact cosine comparisons.
...
| Remote GPU | [`src/marnwick/remote_lama.py`](src/marnwick/remote_lama.py), [`src/marnwick/remote_face_engine.py`](src/marnwick/remote_face_engine.py) | Endpoint validation, certificate retrieval and exact pinning, bounded HTTPS LaMa and face inference, strict response validation, and cancellation |
